...
void Protocol_Upload_Ancs_Config(void);
...
{  
    ...
    if(p_data[1] == 1)   // Open
    {
        ...
        if(Movement_GetState() != MOVEMENT_STATE_RUN && Statemachine_AddState(MEAT_OXYGEN_APP))
        {	
            //打开血氧测量
            return;
        }
        
        if(p_data_manage->ecg_ppg_s.ppg_measure_s.btn == 1 || Movement_GetState() == MOVEMENT_STATE_RUN)
        {
            ble_ack[2] = 0x02;
            Ble_SendDataViaBat(ble_ack, 20);
        }
        else if(p_data_manage->ecg_ppg_s.ppg_measure_s.btn_bp == 1 )
        {           
            ble_ack[2] = 0x01;
            Ble_SendDataViaBat(ble_ack, 20);
        }
...
{
    uint8_t ble_ack[20] = {0x82, 0x00,};

    Ble_SendDataViaBat(ble_ack, 20);      
}

/****************************************************************
** Function name:             Protocol_Measure
** Descriptions:              测量控制
** input parameters:          p_nus：              结构体指针
                              p_data：             数据指针
** output parameters:         无
...
** Created Date:              2016-12-07
****************************************************************/ 
static void Protocol_EcgMeasure(uint8_t action, uint8_t adc_type, uint8_t *data_out)
{
    uint8_t ble_ack[20] = {0x93, 0x01,};
    
    ble_ack[2] = action;
    ble_ack[3] = 0;
	
    ble_ack[4] = 250 & 0xFF;
    ble_ack[5] = (250 >> 8) & 0xFF;	
	
#if ECG_ADC_FREQUENCY_APP_125HZ
    ble_ack[6] = 125 & 0xFF;
    ble_ack[7] = (125 >> 8) & 0xFF;
#else    
    ble_ack[6] = 250 & 0xFF;
    ble_ack[7] = (250 >> 8) & 0xFF;
#endif
    Ble_SendDataViaBat(ble_ack, 20);  
    ...
    {   //开启
        ...
        {
            data_out[0] = 0x03;     // 低电判断
        }
        else if(Movement_GetState() != MOVEMENT_STATE_RUN && Statemachine_AddState(MEAT_ECG_APP_HEART))
        {
            if(adc_type == 1)
            {   // 是否需要发送adc
                p_data_manage->upload = UPLOAD_ADC_ECG_EVENT;
                //重置发送数据
                if(p_data_manage->upload == UPLOAD_ADC_ECG_EVENT)
                {
                    p_data_manage->data_pool.ecg_send_wait_len = 0;
                    p_data_manage->data_pool.ecg_send_read_pos = 0;
                }
            }
            else if(adc_type == 2)
            {   // 是否需要发送adc
                p_data_manage->upload = UPLOAD_ADC_ECG_ORIGINAL_EVENT;
                //重置发送数据
                if(p_data_manage->upload == UPLOAD_ADC_ECG_ORIGINAL_EVENT)
                {
                    p_data_manage->data_pool.ecg_send_wait_len = 0;
                    p_data_manage->data_pool.ecg_send_read_pos = 0;
                }
            }  
            data_out[0] = 0x00;     // 空闲或者可以使用        
        }
        else
        {
            data_out[0] = 0xFF;
...
static void Protocol_Measure(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t ble_ack[20] = {0x93, 0x00,};
    ...
    {   //ecg测量处理
        //判断状态设置是否可以开启ecg测量
        Protocol_EcgMeasure(p_data[2], p_data[3], &ble_ack[4]);       
        ble_ack[2] = p_data[2];
        ble_ack[3] = 1;
...
/****************************************************************
** Function name:             Protocol_ReadEcgDataId
** Descriptions:              读取ECG 数据相关
** input parameters:          p_nus：              结构体指针
                              p_data：             数据指针
** output parameters:         无
** Returned value:            无
...
****************************************************************/ 
static void Protocol_ReadEcgDataId(ble_nus_t *p_nus, uint8_t *p_data)
{    
    uint8_t     ble_ack[20] = {0x00,};
#if 0     
    ...
    uint16_t    time_sum = 0;
#endif     
    ble_ack[0] = p_data[0];  // 0x96   
    ble_ack[1] = p_data[1];  // 
    
#if 0   // WJ.20190327.暂时屏蔽数据保存，新需求，出货     
    time_sum = p_data[2] + p_data[3] + p_data[4] + 
               p_data[5] + p_data[6] + p_data[7] + p_data[8];
    
    memcpy(time_data, &p_data[2], 7);
    if((p_data[1] == 0x01 || p_data[1] == 0x02 || p_data[1] == 0x03)
       && (time_sum == 0 || SystemTime_Check1(time_data, 7) == NRF_SUCCESS))
    {
        ComThread_SendEcgIdData(p_data[1], &p_data[2]);
    }
    else
#endif        
    {
...
** Created Date:              2018-11-06
****************************************************************/ 
static void Protocol_ReadEcgDataById(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t     ble_ack[20] = {0x97, 0x00};      //0x97
    
    ble_ack[0] = p_data[0];
    ble_ack[1] = p_data[1];

#if 0   // WJ.20190327.暂时屏蔽数据保存，新需求，出货   
    if(p_data[1] == 2 || p_data[1] == 3)
    {
        ComThread_SendEcgDataById(p_data[1], (p_data[2] + (p_data[3]<<8)), NULL);
    }
    else
#endif        
    {
...
//        else
//        { 
//            if(p_data[7] <= 0 || p_data[7] > CONFIG_BACKLIGHT_MAX_LEVEL
//              || p_data[6] <= 0 || p_data[6] > CONFIG_BACKLIGHT_MAX_LEVEL)
//            {
//                ble_ack[1] = 0;     //数据不合理
//            }
//            else
...
//                if(p_data[8] == 1)
//                {   //自动
//                    st_bright.other_bright = CONFIG_BACKLIGHT_MAX_LEVEL_RECOMMEND;
//                    st_bright.valid_bright = CONFIG_BACKLIGHT_MIN_LEVEL_RECOMMEND;
//                    st_bright.start_hour = 22;
//                    st_bright.start_minute = 0;
//                    st_bright.end_hour = 8;
//                    st_bright.end_minute = 0; 
//                    ble_ack[9] = 1;                
//                }
//                else
//                { 
...
//    
//    if(p_flash_param->brightness_s.start_hour == 22 
//       && p_flash_param->brightness_s.start_minute == 0
//       && p_flash_param->brightness_s.end_hour == 8
//       && p_flash_param->brightness_s.end_minute == 0)
//    {
//        ble_ack[9] = 1;    
//    } 
//    else
...
//    }    
//    
//    ble_ack[10] = BACKLIGHT_MAX_CHANAGE_LEVEL;
//    
//    Ble_SendDataViaBat(ble_ack, 20);   
//}

/****************************************************************
** Function name:           Protocol_BrightScreenTimeSet
** Descriptions:            屏幕亮屏时间设置
...
*****************************************************************/ 
static void Protocol_BrightScreenTimeSet(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t ble_ack[20] = {0x00, };
    
    ble_ack[0] = p_data[0];  // 0xB4   
    ble_ack[1] = 1;     
    ble_ack[2] = p_data[1];  
    if(p_data[1] == 0x01)
    {
        if(p_data[2] < UI_DISPLAY_TIME_MIN || p_data[2] > UI_DISPLAY_TIME_MAX)
        {
            ble_ack[1] = 0;
        }
        else
        {
            if(p_flash_param->ui_display_time != p_data[2])
            {   
                Api_UpdateUiDisplayTime(p_data[2]); 
            }    
        }        
    }
    
    ble_ack[3] = p_flash_param->ui_display_time;
    ble_ack[4] = UI_DISPLAY_TIME_RECOMMEND;
    ble_ack[5] = UI_DISPLAY_TIME_MAX;      
    ble_ack[6] = UI_DISPLAY_TIME_MIN;    
...
static void Protocol_Camera(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if((Battery_Get_Status() != BATTERY_POWER_NORMAL)|| (GuiThread_IsDuringFindPhone())
	   || (task_id_tmp == CPU_TASK_HEART) || Movement_IsBusy()
	   || (task_id_tmp == CPU_TASK_ECG)
	   || (task_id_tmp == CPU_TASK_BP)
       || (task_id_tmp == CPU_TASK_OXYGEN)
...
static void Protocol_SwitchState(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_data[1] == 1)
    {
        ...
        if(p_data[16] == 1)
        {
            dev_config_s.ppg_detect_enable = 1;
        }
        else if(p_data[16] == 2)
        {
	        if(dev_config_s.ppg_detect_enable == 1)
            {  // 如果正在自动测测量，关闭测量 
                Statemachine_DelState(MEAT_PPG_TIME); 
            }
            dev_config_s.ppg_detect_enable = 0;
        }  
...
static void Protocol_Upload_Fun_Config(void)
{
    ...
    ble_ack[2] = 0x00;  //饮酒
    if(p_flash_param->sit_func_state)
    {        
        ble_ack[3] = 0x01; //久坐
    }
    else
    ...
    ble_ack[12] = 0x00; //女性
    ble_ack[13] = 0x00;//0x01; //屏幕亮度设置
    ble_ack[14] = 0x00; //0xC3指令相关
    ble_ack[15] = 0x01; //手环精度标志位
#if (APP_USED_OLD_ALARM_ENABLE)
    ble_ack[17] = 0x00;//0x03; //新闹铃协议  0x01
#else
    ble_ack[17] = 0x03; //新闹铃协议  0x01
    ...
    ble_ack[0] = 0xA7;
    ble_ack[1] = 0x01;  //0x01;  //倒计时功能
    ble_ack[3] = 0x04;  //0x02;  //信息推送备注包数
    ble_ack[5] = 0x03;  //运动模式存数次数
	ble_ack[6] = MAIN_STYLE_NUM;    //支持5种风格
    ble_ack[7] = 0x00;  //0x02;  //呼吸率
    ble_ack[8] = 0x01;  //HRV
    ble_ack[9] = 0x00;  //天气
    ble_ack[10] = 0x03; //协议上传方式，app需要解析
    ble_ack[11] = 0x01; //A5指令完善
    ble_ack[12] = 0x01; //亮屏时长
    ble_ack[13] = 0x01; //睡眠类型，精准睡眠
    ble_ack[14] = 0x01; //清除数据 
    ble_ack[15] = 0x01; //ECG
    ble_ack[16] = 0x01; //10中运动模式
...
{   
    ...
    //WJ.20181022.app隐藏佩戴开关，设备端匹配肤色是否使能佩戴测量
    ble_ack[19] = 0;
...
{ 
    ...
    if(p_data[1] == 0x01)
    {
		...
		//手环已启动，被占用，设备忙碌
        if(Statemachine_AddState(MEAT_PPG_APP_HEART))
        {
			p_data_manage->upload = UPLOAD_ADC_GREEN_EVENT;
            //重置发送数据
            if(p_data_manage->upload == UPLOAD_ADC_GREEN_EVENT)
            {
                p_data_manage->data_pool.ppg_send_wait_len = 0;
                p_data_manage->data_pool.ppg_send_read_pos = 0;
            }            
        }
        else
        {
...
static void Protocol_GetMovementPatternsDataCRC(ble_nus_t *p_nus, uint8_t *p_data)   
{
    ...
    ble_ack[2] = 0x01; 
    if(p_data[1] == 0x01)
    {   //WJ.20190108.由于需要串行处理，把数据处理移到通讯任务中
        /////Movement_GetHeadCRC(&ble_ack[3], 3);
        ComThread_SendMovementData(1, 0);
        return;
    }
...
static void Protocol_GetMovementPatternsData(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    {   //WJ.20190108.由于需要串行处理，把数据处理移到通讯任务中
        ret = 1;/////Movement_SetSendDataInfo(p_data[1]);        
    }
    
    if(ret == 1)
    {
        //上传运动模式数据
        ComThread_SendMovementData(0, p_data[1]);
    }
    else
    {       
        ble_ack[0] = p_data[0]; 
...
**----------------------------------------------------------------
*****************************************************************/ 
extern uint8_t runmode_show_type;  //运动模式显示类型，为2页显示；
...
static void Protocol_OpreateMovementPatternsData(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    else if(p_data[1] == 1)
    {
        ...
        if(Gui_DataStopWatchIsBusy() 
          || Battery_Get_Status() == BATTERY_POWER_CHARGE 
          || Battery_Get_Status() == BATTERY_POWER_FULL
          || p_data[2] == 0 || p_data[2] > 10)
        {
            ble_ack[2] = 0x00; 
            ble_ack[3] = 0x02; 
            ble_ack[4] = p_data[2];
        ...
        {
            //Movement_Start();
            //Movement_AddSaveOptEvent(1);
            runmode_type = (MOVEMENT_SELECT_MODE_TYPE_E)p_data[2];
            ble_ack[2] = 0x01; 
            ble_ack[3] = 0x00; 
            ble_ack[4] = runmode_type;
            
            Gui_MovementDisplayInit();
            
            //如果没有开启运动模式则开启运动模式
            if(!Gui_DataMovementModeIsRun() )
            {
                Gui_DataStartMovementMode();
            }
            if(runmode_type == APP_MSM_MODE_TYPE_TREADMILLS_MACHINE
               || runmode_type == APP_MSM_MODE_TYPE_BIKING_OUTDOOR
               || runmode_type == APP_MSM_MODE_TYPE_BIKING_INDOOR
               || runmode_type == APP_MSM_MODE_TYPE_ELLIPTICAL_MACHINE
...
static void Protocol_SetSedentaryTime(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_flash_param->sit_func_state != 1)
    {
        Ble_SendDataViaBat(ble_ack, 20); 
        return;
    }   
...
static void Protocol_TestFunctional(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    {  
        ...
        Statemachine_AddState(MEAT_PPG_APP_HEART);
        p_data_manage->upload = UPLOAD_ADC_GREEN_EVENT;
...
#else
static void Protocol_SetOldAlarm(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t ack_data[20] = {0xAB, 0x00, };  
    uint8_t alarm_old_time_buf[9] = {0};    
    
    if(p_data[10] == 0x00)
    {   //清除手环振动闹钟设置
        memset(alarm_old_time_buf, 0x00, sizeof(alarm_old_time_buf));      
        // 保存   
        memcpy(p_flash_param->alarm_old_time, alarm_old_time_buf, 9);
        BleThread_AsynUpdateAppConfig();     
        
        ack_data[1]  = 0x01;	
        ack_data[11] = 0x00;	    
    }
    else if(p_data[10] >= 0x01 && p_data[10] <= 0x05)
    {
        if(((p_data[1] < 24) && (p_data[2] < 60) ) 
           && ((p_data[4] < 24) && (p_data[5] < 60))
           && ((p_data[7] < 24) && (p_data[8] < 60)))
        {
            alarm_old_time_buf[0] = p_data[1];//时
            alarm_old_time_buf[1] = p_data[2];//分
            alarm_old_time_buf[2] = p_data[3];//振动方式  
            
            alarm_old_time_buf[3] = p_data[4];//时
            alarm_old_time_buf[4] = p_data[5];//分
            alarm_old_time_buf[5] = p_data[6];//振动方式 
            
            alarm_old_time_buf[6] = p_data[7];//时
            alarm_old_time_buf[7] = p_data[8];//分
            alarm_old_time_buf[8] = p_data[9];//振动方式    
        ...
        ack_data[11] = p_data[10];          
    }
    else if(p_data[10] == 0x06)
    {   // 读取
        memcpy(&ack_data[2], p_flash_param->alarm_old_time, 9);
        ack_data[1] = 0x01;
        ack_data[11] = p_data[10];   
    }
    else
...
//            {
//                p_flash_param->sit_func_state = (ble_ack[2] == 0x01?1:0);
//                BleThread_UpdateSitFuncConfig();               
//                result = ble_ack[2];
//            }
//            else if(ble_ack[2] == 0x03)
//            {
//                result = (p_flash_param->sit_func_state == 0?2:1);
//            }
//        }
//        ble_ack[7] = result;               
//    }
...
static void Protocol_BloodPressTest(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    {   
        ...
        else if(Statemachine_AddState(MEAT_PPG_APP_BP))
        {
            //WJ.20190105.屏蔽波形发送
            ...
            p_data_manage->app_bp_mode = (p_data[2] == 1?1:0);
            ble_ack[4] = 0x00;    
        }
        else
        {   //测量忙碌
            ble_ack[4] = 0x01;//0xFF;     // 忙碌中                                 
        }
    }
    else if(p_data[1] == 0x00)
    {
...
/****************************************************************
** Function name:           Protocol_SendOldDailyData
** Descriptions:            旧协议读取日常数据
** input parameters:        p_nus：数据接收通道，p_data:接收的数据
** output parameters:       无
** Returned value:          无
** Created by:              WJ              
...
*****************************************************************/ 
static void Protocol_SendOldDailyData(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t     ble_ack[20] = {0x00, };
    ...
    ble_ack[3] = 0x00;   
    ble_ack[4] = 0x00;    
    ble_ack[5] = (p_data[3]<<5) & 0xE0;

    Ble_SendDataViaBat(ble_ack, 20);   
}

/****************************************************************
** Function name:           Protocol_SendOldScatterHeartData
** Descriptions:            设备不支持--读取HRV数据
** input parameters:        p_nus：数据接收通道，p_data:接收的数据
** output parameters:       无
** Returned value:          无
** Created by:              WJ              
** Created Date:            2018-11-15
*****************************************************************/ 
static void Protocol_SendOldScatterHeartData(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t     ble_ack[20] = {0x00, };
    
    ble_ack[0] = p_data[0];  // 0xD9
    ble_ack[1] = 0x00;   
    ble_ack[2] = 0x00;
    ble_ack[3] = 0x00;   
    ble_ack[4] = 0x00;    
...
/****************************************************************
** Function name:           Protocol_SendOldOxygenData
** Descriptions:            设备不支持--读取血氧数据
** input parameters:        p_nus：数据接收通道，p_data:接收的数据
** output parameters:       无
** Returned value:          无
** Created by:              WJ              
** Created Date:            2018-11-15
*****************************************************************/ 
static void Protocol_SendOldOxygenData(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t     ble_ack[20] = {0x00, };
    
    ble_ack[0] = p_data[0];  // 0xD2
    ble_ack[1] = 0x00;   
    ble_ack[2] = 0x00;
    ble_ack[3] = 0x00;   
    ble_ack[4] = 0x00;    
...
** Descriptions:            外设调试使用，加速度、佩戴、4900
** input parameters:        p_nus：数据接收通道，p_data:接收的数据
** output parameters:       无
** Returned value:          无
** Created by:              WJ             
...
static void Protocol_PeripheralTest(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_data[1] == 0x05 || p_data[1] == 0x06)
    {   //1182 ECG设置   
        if(p_data[1] == 0x05)
        {
            Si1182_EcgParamSet(p_data[2], p_data[3], p_data[4], p_data[5],
							p_data[6], p_data[7], p_data[8], p_data[9],
							p_data[10], p_data[11], p_data[12], p_data[13],
							p_data[14], p_data[15], p_data[16], p_data[17]);
        }
   
		ble_ack[2] = 1;
        Si1182_EcgParamGet(&ble_ack[3], &ble_ack[4], &ble_ack[5], &ble_ack[6],
						&ble_ack[7], &ble_ack[8], &ble_ack[9], &ble_ack[10],  
						&ble_ack[11], &ble_ack[12], &ble_ack[13], &ble_ack[14],
						&ble_ack[15], &ble_ack[16], &ble_ack[17], &ble_ack[18]);
		
		Ble_SendDataViaBat(ble_ack, 20);
    }
	
	if(p_data[1] == 0x07 || p_data[1] == 0x08)
    {   //1182 PPG设置   
        if(p_data[1] == 0x07)
        {
			if(p_data[2] == 1)
			{
				Si1182_PpgParamSet1(p_data[3], p_data[4], p_data[5],
									p_data[6], p_data[7], p_data[8], p_data[9],
									p_data[10], p_data[11], p_data[12], p_data[13],
									p_data[14], p_data[15], p_data[16], p_data[17],
									p_data[18]);
			...
			{
				Si1182_PpgParamSet2(p_data[3], p_data[4], p_data[5],
									p_data[6], p_data[7], p_data[8], p_data[9],
									p_data[10], p_data[11], p_data[12], p_data[13],
									p_data[14], p_data[15], p_data[16], p_data[17],
									p_data[18]);
		...
		{
			ble_ack[3] = 1;
			Si1182_PpgParamGet1(&ble_ack[4], &ble_ack[5], &ble_ack[6],
							    &ble_ack[7], &ble_ack[8], &ble_ack[9], &ble_ack[10],  
							    &ble_ack[11], &ble_ack[12], &ble_ack[13], &ble_ack[14],
							    &ble_ack[15], &ble_ack[16], &ble_ack[17], &ble_ack[18],
							    &ble_ack[19]);
		...
		{
			ble_ack[3] = 2;
			Si1182_PpgParamGet2(&ble_ack[4], &ble_ack[5], &ble_ack[6],
							    &ble_ack[7], &ble_ack[8], &ble_ack[9], &ble_ack[10],  
							    &ble_ack[11], &ble_ack[12], &ble_ack[13], &ble_ack[14],
							    &ble_ack[15], &ble_ack[16], &ble_ack[17], &ble_ack[18],
								&ble_ack[19]);
	...
	if(p_data[1] == 0xff)
	{
		...
		Si1182_SetParameterReadFlag();
		Ble_SendDataViaBat(ble_ack, 20);
	}
	else if(p_data[1] == 0xfe)
	{
		for(uint32_t i = 0; i < 6; i++)
		{
			Si1182_GetParameterConfig(ble_ack, i);
			Ble_SendDataViaBat(ble_ack, 20);
			nrf_delay_ms(20);
		}
	}
...
/****************************************************************
** Function name:           Protocol_PowerInfoDeal
** Descriptions:            用于功耗统计
** input parameters:        p_nus：数据接收通道，p_data:接收的数据
** output parameters:       无
** Returned value:          无
...
*****************************************************************/ 
static void Protocol_PowerInfoDeal(ble_nus_t *p_nus, uint8_t *p_data)
{
    uint8_t             ble_ack[20] = {0x00,};
    ...
    if(p_data[1] == 0x01)
    {
        ComThread_SendPowerInfoData(p_data[2]);
        return;
    }
...
void Protocol_Bat_Data_Handler(ble_nus_t *p_nus, uint8_t *p_data, uint16_t length)
{
    ...
    { 
        switch(p_data[0])
        {
            ...
            //    break;
            case 0x96:  //读取ECG数据ID
                Protocol_ReadEcgDataId(p_nus, p_data);
                break;
            
            case 0x97:  //读取ECG数据,通过ID
                Protocol_ReadEcgDataById(p_nus, p_data);
                break;
...
#if (APP_USED_OLD_ALARM_ENABLE)            
            case 0xAB:  //三组闹钟模式
                Protocol_SetOldAlarm(p_nus, p_data);
                break;
#endif //APP_USED_OLD_ALARM_ENABLE
            ...
            case 0xB4:
                //屏幕亮屏时间设置
                Protocol_BrightScreenTimeSet(p_nus, p_data);
                break;
            case 0xB5:  //手环查找手机功能
            ...
            case 0xB9://闹钟设置
                ...
                break;        
#endif //APP_USED_OLD_ALARM_ENABLEs
...
//                break; 
            ...
            case 0xD0://读取心率 
                ...
                break;
            case 0xD1:  //发送日常数据(旧协议不支持)
                Protocol_SendOldDailyData(p_nus, p_data);
                break;
            case 0xDF:  
                ComThread_SendDailyData(p_data);
                break;
            
            case 0xD2:  //不支持此协议-设备不支持（发送血氧数据）
               Protocol_SendOldOxygenData(p_nus, p_data);
               break;
            ...
            case 0xD9:  //不支持此协议-设备不支持
                Protocol_SendOldScatterHeartData(p_nus, p_data);
                break;
            ...
            case 0xE1:  //Set the sedentary remind time period///设置久做时间 
                ...
                break;            
            case 0xEF:  // 功耗统计
                Protocol_PowerInfoDeal(p_nus, p_data);
                break;
			...
			case 0xFF:
                ...
                else if(p_data[1] == 0xFF)
                {
			        //测试用，手动擦除全部FLASH
                    uint8_t Extflash_AllOptEraseAllSet(void);
                    Extflash_AllOptEraseAllSet();
