...
{    
    ...
    for(uint32_t i = 0; i < p_call_notify->len; i++)
    {
        if(p_call_notify->content[i] == 0)
        {
            p_call_notify->len = i;
            break;
        }
    ...
    if(p_call_notify->content[0] == 226 
       && p_call_notify->content[1] == 128
       && p_call_notify->content[2] == 173)
    {
        p_call_notify->len -= 3;
        memcpy(p_call_notify->content, (uint8_t *)&p_call_notify->content[3], p_call_notify->len);
    }
    if(p_call_notify->content[p_call_notify->len - 3] == 226 
       && p_call_notify->content[p_call_notify->len - 2] == 128
       && p_call_notify->content[p_call_notify->len - 1] == 172)
    {
        p_call_notify->len -= 3;
        memset(&p_call_notify->content[p_call_notify->len], 0, 6);
    }   
}
