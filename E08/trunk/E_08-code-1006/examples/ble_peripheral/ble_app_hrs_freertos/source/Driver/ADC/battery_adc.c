...
#define BATTERY_SYSTEMBOOT_ADC  2873  //3.3674V
...
**                            计算公式: ADC = 4096*v/4.8;  ADC = V* 853.3333333333333
** input parameters:          无
** output parameters:         无
** Returned value:            无
** Created by:                GT
...
void Battery_PowerOnProcess(void)
{
    ...
    if(battery_average < 3116) //3.6517
    {
        //st_battery.bat_grade_low_cnt = BATTERY_CHECK_COUNT;
        st_battery.grade = 0;
    }
    else if(battery_average < 3193)//3.7422
    {
        st_battery.grade = 1;
    }
    else if(battery_average < 3248) //3.8061
    {
        st_battery.grade = 2;
    }
    else if(battery_average < 3369) //3.9477
    {
        st_battery.grade = 3;
    }
    else
...
{    
    ...
    // low power off  add by hyw 201701016
	if(battery_adc_gather < 3098 )//3.6309V
    {
		st_battery.bat_grade_low_cnt ++;
        st_battery.bat_grade0_cnt = BATTERY_CHECK_COUNT;//电池显示0格
        st_battery.bat_grade1_cnt = 0;
        st_battery.bat_grade2_cnt = 0; 
        st_battery.bat_grade3_cnt = 0;
        st_battery.bat_grade4_cnt = 0;
	}
    else if(battery_adc_gather < 3116)//3.6517V
    {
		st_battery.bat_grade_low_cnt = 0;
    ...
    else if(battery_adc_gather < 3193)//3.7422V
    {
		st_battery.bat_grade_low_cnt = 0;
    ...
    else if(battery_adc_gather < 3248)//3.8061V
    {
		st_battery.bat_grade_low_cnt = 0;
        st_battery.bat_grade0_cnt = 0;
        st_battery.bat_grade1_cnt = 0;
    ...
    else if(battery_adc_gather < 3369)//3.9477V
    {
		st_battery.bat_grade_low_cnt = 0;
        st_battery.bat_grade0_cnt = 0;
        st_battery.bat_grade1_cnt = 0;
    ...
    // WJ.20190114.防止低电闪烁到一格的跳跃    
    if(st_battery.grade == 0 && st_battery.status == BATTERY_POWER_NORMAL)
    {
        st_battery.bat_grade1_cnt = 0;
        //st_battery.bat_grade2_cnt = 0;
        //st_battery.bat_grade3_cnt = 0;
    ...
    if((0 == nrf_gpio_pin_read(CHARGING_PIN)) && (0 == nrf_gpio_pin_read(CHG_PIN)))
    {
        ...
        //Movement_Exit();
        Movement_AddSaveOptEvent(3);
        
        /* 充电*/
        Vnwd_SetWear(6);
        //WJ.20181127.充电重置久坐计时
        p_data_manage->sedentary_cnt = 0;
        
        p_data_manage->power_statistics_s.charging = 1;
    }
    ...
    else
    {
        ...
        {
            //关闭加速度
            BleThread_GSensorStop();//Gsensor_Stop();
        }
...
/****************************************************************
** Function name:           Battery_IsBootBatteryAdc
** Descriptions:            电池adc采样数据,单次，用于上电检测
** input parameters:        无  
** output parameters:       无
...
*****************************************************************/
uint8_t Battery_IsBootBatteryAdc(void)
{
    int16_t battery_adc = 0;
    
    nrf_drv_saadc_sample_convert(0, (nrf_saadc_value_t *)&battery_adc);
    
    if(nrf_gpio_pin_read(CHARGING_PIN) == 0x01)
    {
        return 1;
    }
    ...
    if(battery_adc > BATTERY_SYSTEMBOOT_ADC)
    {
        return 1;
    }
    return 0;
...
/****************************************************************
** Function name:           Battery_ResetBatteryAdc
** Descriptions:            用于上电检测第一次正式电压
** input parameters:        无  
** output parameters:       无
** Returned value:          无
** Created by:              WJ              
...
*****************************************************************/
void Battery_ResetBatteryAdc(void)
{
    Battery_PowerOnProcess();
    if(st_battery.grade == 0)
    {
