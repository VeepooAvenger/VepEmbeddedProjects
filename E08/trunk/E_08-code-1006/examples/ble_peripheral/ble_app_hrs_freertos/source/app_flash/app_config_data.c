...
uint32_t App_GetDevConfig(dev_config_t* p_dev)
{
    ...
    else
    {
        ...
        p_dev->alarm_enable = 0x01;        // 闹钟启用
        p_dev->watchtimer_enable = 0;
        p_dev->low_bo_alert_enable = 1;
        
        p_dev->ecg_detect_enable = 0;
        p_dev->ppg_detect_enable = 0;//1;   //WJ.20190718.24小时脉率开关关闭（app科学睡眠开关）
        
        ///p_dev->precise_sleep = 1;
    }
...
uint32_t App_GetBrightnessParam(brightness_config_t* p_param)
{
    ...
    {
        p_param->other_bright = CONFIG_BACKLIGHT_MAX_LEVEL_RECOMMEND;
        p_param->valid_bright = CONFIG_BACKLIGHT_MIN_LEVEL_RECOMMEND;
...
/*******************************************************************************************************
** Function name:            App_UpdateUiDisplayTimeParam
** Descriptions:             更新亮屏保持时间按参数
** input parameter:          ui_display_time：设置参数
** Returned Value:           NRF_SUCCESS
** Created  by:              WJ
...
{
    uint32_t    ui_time_config[3] = {0};
    uint8_t     *p_ui_time_config = (uint8_t*)ui_time_config;

    memcpy(&ui_time_config[0], "UID_Time", 8);
    //亮屏时间保持
    p_ui_time_config[8] = ui_display_time;

    return Flash_CommonStore((uint32_t*)(APP_CONFIG_BASE_ADDR + 0x0104), ui_time_config, 3);
}

/*******************************************************************************************************
** Function name:            App_GetUiDisplayTimeParam
** Descriptions:             获取亮屏保持时间按参数
** input parameter:          ui_display_time：设置参数
** Returned Value:           NRF_SUCCESS
** Created  by:              WJ
** Created  Datas:           2018-08-21
...
uint32_t App_GetUiDisplayTimeParam(uint8_t *ui_display_time)
{
    ...
    /* 从FLASH中读取数据, 偏移量需要参考文档计算 */
    Flash_CommonRead(ui_time_config, APP_CONFIG_BASE_ADDR + 0x0104, sizeof(ui_time_config));
    
    /* 校验读取头部标志 */
    if(memcmp(&ui_time_config[0], "UID_Time", 8) == 0)
    {
        /* 如果读取校验正确 */
        *ui_display_time = p_ui_time_config[8];
    }
    else
    {
        *ui_display_time = UI_DISPLAY_TIME_RECOMMEND;
    }
