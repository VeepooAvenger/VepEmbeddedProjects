        ...
        if(p_event->gui_controller.animation_tick_counter%5 == 0)
         {
			 ...
			 Gui_PageBatteryCharg(p_event->gui_controller.animation_frame_index++, 1, &p_device->cls_flag);
         }
    }
    else 
    {
...
void Cpu_AlarmHintEvent(cpu_event_tcb_t* p_event, cpu_device_tcb_t* p_device)
{
    ...
    {    
		...
		Gui_PageAlarm(p_event->gui_controller.animation_frame_index++,am_pm_format, p_data_manage->alarm_scence,display_hour, clock->minutes, display_language,
		               &p_device->cls_flag);
...
void Cpu_SedentaryHintEvent(cpu_event_tcb_t* p_event, cpu_device_tcb_t* p_device)
{
    ...
    if(p_event->gui_controller.animation_tick_counter%2 == 0)
    {    
        p_event->gui_controller.page_frame_counter = clock->seconds;
		Gui_PageSit(p_event->gui_controller.animation_frame_index++, display_language, &p_device->cls_flag);
...
void Cpu_CallHintEvent(cpu_event_tcb_t* p_event, cpu_device_tcb_t* p_device)
{
    ...
    {      
		...
		Gui_PageCallNotificationIcon(p_event->gui_controller.animation_frame_index, last_call_state, display_language, &p_device->cls_flag);
        //这里需要分类处理
        if(last_page_index == 0xFF && (last_call_state == GUI_CALL_INCOMING || last_call_state == GUI_CALL_MUTE))
        {
            Gui_PageCallNotificationContent(p_event->gui_controller.animation_frame_index, p_call_notify->content, p_call_notify->len, &p_device->cls_flag);
        }
    ...
    {	
        if(p_message_notify->len > 0)
        {
            ...
            if(p_message_notify->len >= p_data_manage->app_notify_s.display_len)
            {
                ...
                //WJ.20180904.对显示数据检查，是否能正常显示
                if(p_message_notify->len <= 4)
                {
...
//		Gui_PageMessageNotificationIcon(p_message_notify->type,p_event->gui_controller.animation_frame_index++);
		...
		p_data_manage->app_notify_s.display_len = Gui_PageMessageNotificationContent(p_event->gui_controller.animation_frame_index++, p_message_notify->content, p_message_notify->len, &p_device->cls_flag);
        ...
        else   //YX.20180427.解决读取完推送消息，点击下一界面后UI显示问题，会闪屏一次
        {
            ...
            Gui_PageMessageNotificationIcon(p_event->gui_controller.animation_frame_index++, p_message_notify->type, 0, hour,clock->minutes, &p_device->cls_flag);
        }
    }
...
/****************************************************************
** Function name:            Cpu_LowOxygenAlertHintEvent
** Descriptions:             // 低氧报警
** input parameters:         p_controller：         CPU任务控制器指针
                             p_gui：                静态页面控制器指针
** output parameters:        无
...
//        Gui_PageClearBackground(0, 0, LCD_MAX_COLUMN, LCD_MAX_ROW);
    ...
    if(p_event->gui_controller.animation_tick_counter%6 == 0)
    {
        Gui_LowOxygenAlertPage(p_event->gui_controller.animation_frame_index++, &p_device->cls_flag);
        //设置清屏参数
...
//                                p_device->cls_param.column_size, p_device->cls_param.page_size);      
    ...
    if(p_event->gui_controller.animation_tick_counter%3 == 0)
    {
//		Gui_PageCountDown(true, p_event->gui_controller.animation_frame_index++);
        hour = Hour_StandardTransform(p_flash_param->dev_config_s.am_pm_enable,clock->hour);
        Gui_PageCountDown(p_event->gui_controller.animation_frame_index++, 1, p_flash_param->hourglass_s.run_remain_time, 
...
//                                p_device->cls_param.column_size, p_device->cls_param.page_size);        
    ...
    if((p_event->gui_controller.animation_tick_counter%3) == 0)
    {
//		Gui_PageCountDownTimeOut(p_event->gui_controller.animation_frame_index++, 1, &p_device->cls_flag);
        hour = Hour_StandardTransform(p_flash_param->dev_config_s.am_pm_enable,clock->hour);
        Gui_PageCountDown(p_event->gui_controller.animation_frame_index++, 1, p_flash_param->hourglass_s.run_remain_time, 
                          hour, clock->minutes, display_language, &p_device->cls_flag,0);         
...
{	
    ...
    if(p_event->task_controller.init == 1)
    {
        Cpu_EventParamInit(p_event,1000,false,true);
...
//        Gui_PageClearBackground(0, 0, LCD_MAX_COLUMN, LCD_MAX_ROW);
    ...
    if(p_event->gui_controller.animation_tick_counter%3 == 0)
    {
		Gui_PageCameraReady(p_event->gui_controller.animation_frame_index++, /*camera_flg*/camera_state, &p_device->cls_flag);
        ...
        if(p_event->gui_controller.animation_frame_index > 100)
        {
            camera_flg = 0;
        }
    ...
    if(((camera_response_enable)&&((Vph_GetShakeFlag()>2 && camera_delay_cnt >= 2) || p_data_manage->turnwrist_camera_state == 1))||(p_device->key == BUTTON_EVENT_CLICK))
    {
        ...
        p_event->gui_controller.animation_frame_index = 1;
...
//                                p_device->cls_param.column_size, p_device->cls_param.page_size);      
    ...
    if(p_event->gui_controller.animation_tick_counter % 3 == 0 )//300ms 6
    {
		Gui_PageStarget(p_event->gui_controller.animation_frame_index++, p_flash_param->sport_target, display_language, &p_device->cls_flag);
