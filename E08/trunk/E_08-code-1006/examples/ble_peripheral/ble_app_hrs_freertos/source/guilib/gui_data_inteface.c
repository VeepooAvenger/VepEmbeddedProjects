...
{
    return p_data_manage->device_trace_param_s.ble_connect;//system_flag_s.ble_connect;
}
...
{
    if((p_flash_param->hourglass_s.app_trigger == 1)
...
////    p_flash_param->hourglass_s.run = 0;
////}
...
** Created Date:            2017-07-28
*****************************************************************/ 
////void Gui_DataCountDownStart(uint32_t countdown_second)
////{
////    //p_flash_param->hourglass_s.app_send = 1;
////    p_flash_param->hourglass_s.btn_trigger = 1;
////    p_flash_param->hourglass_s.run_remain_time = countdown_second;
////    
////    p_flash_param->hourglass_s.last_set_time = countdown_second;   
////    p_flash_param->hourglass_s.run = 1;    
////}
...
////{
////    if(p_flash_param->hourglass_s.run == 0)
////    {
////        p_flash_param->hourglass_s.run = 1;
////    }
////    else
...
{
//    if(p_flash_param->hourglass_s.last_set_time == 0)
//    {
//        p_flash_param->hourglass_s.last_set_time = p_flash_param->hourglass_s.dev_remain_time;
//    }
//    return p_flash_param->hourglass_s.last_set_time;//Countdown_GetCurrentSecond(); 
    return p_flash_param->hourglass_s.run_remain_time;
...
/****************************************************************
** Function name:           Gui_MovementDisplayInit
...
*****************************************************************/ 
void Gui_MovementDisplayInit(void)
{
    Movement_DisplayInit();
}
...
** Created Date:            2017-07-28
*****************************************************************/ 
uint16_t Gui_DataGetSportsToMarkProgress(void)
{
    uint16_t step_mark = 0;
    uint32_t all_step = 0;                
    uint32_t distance_m = 0;             
    float   distance_km = 0;              
    float   distance_mile = 0;           
    uint32_t   calories_buf = 0;          
      
    step_mark = p_flash_param->sport_target;
    
    distance_value(&all_step,&distance_m,&distance_km,&distance_mile,&calories_buf);

    if(all_step >= step_mark)
    {
        return 100;
    }    
    return (all_step*100)/step_mark;    
}
/****************************************************************
** Function name:           Gui_CalcMessageDispInfo
** Descriptions:            计算消息推送的显示信息
** input parameters:        无
** output parameters:       无
** Returned value:          无
** Created by:              YX              
...
*****************************************************************/ 
uint8_t Gui_CalcMessageDispInfo(page_message_param_t* message_para)
{
    uint8_t message_len = 0;
//    uint8_t pos_tmp = p_flash_param->app_message_attr_s.message_now_pos;
    
//    message_para->disp_data_num = p_flash_param->app_message_attr_s.message_num;
//    for(uint8_t i = 0; i < message_para->disp_data_num; i++)
//    {
//        message_para->message_type[i] = p_flash_param->app_message_attr_s.message_notify_attr_s[pos_tmp].type;
//        message_para->message_time_s[i] = p_flash_param->app_message_attr_s.message_notify_attr_s[pos_tmp].message_time_s;
//        pos_tmp = (p_flash_param->app_message_attr_s.message_now_pos+i)%3;
//        if(p_flash_param->app_message_attr_s.message_notify_attr_s[pos_tmp].len >= GUI_DISP_MESSAGE_MAXLEN)
//        {
//            message_len = GUI_DISP_MESSAGE_MAXLEN;
//        }
//        else
//        {
//            message_len = p_flash_param->app_message_attr_s.message_notify_attr_s[pos_tmp].len;
//        }
//        message_para->content_len[i] = message_len;
//        memcpy(message_para->p_message_content[i], p_flash_param->app_message_attr_s.message_notify_attr_s[pos_tmp].content, 
//               message_para->content_len[i]);
//    }
    return 0;
    
}
/****************************************************************
** Function name:           Gui_DataGetBleName
** Descriptions:            获取蓝牙名称
** input parameters:        无
** output parameters:       bleName:蓝牙名称  最多8位
** Returned value:          无
** Created by:              ZZZ             
** Created Date:            2019-06-01
*****************************************************************/ 
void Gui_DataGetBleName(uint8_t *bleName)
{
    if(m_ble_name[0] != 0x00)
    {
         memcpy(bleName, m_ble_name, strlen((char *)m_ble_name));
    }
    else
    {
         memcpy(bleName,(const uint8_t *)DEVICE_NAME,
                                          strlen(DEVICE_NAME));
    }
}
