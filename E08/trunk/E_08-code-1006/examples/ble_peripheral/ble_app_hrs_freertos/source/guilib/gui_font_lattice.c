...
//    }
//    
//    //0xBF01:全0xFF //字库：0x387C // flash字库：0x3D18
...
static uint8_t Font_CodeByteNum(uint8_t *p_data)
{
    ...
    /* UTF8编码 */
    //YX.20180514.备注名有无法显示的内容时，读取完推送消息会继续显示推送UI（必现）（BUG：2577）
...
//        Font_ImageLeftRotate((void*)code, rotate_code, 16, width);
        if(column + width > LCD_MAX_COLUMN)
        {
            page += 16 + 7;
            column = LCD_FONT_INTERVAL;
        } 
        //WJ.20171229.限制显示范围        
        if(page > (LCD_MAX_ROW - 16))
        {
            return bRet;
        }            
        if(width == 16)
...
{       
    ...
    //起始页   
    page = start_page;   
    if((max_width * 8 + (font_num - 1) * LCD_FONT_INTERVAL) <= LCD_MAX_COLUMN)
    {
        //起始列
        pos = (LCD_MAX_COLUMN - (max_width * 8 + (font_num - 1) * LCD_FONT_INTERVAL)) / 2;
    }
    else
    { 
        //起始列
    ...
    {
        if((p_content[i] == 0x00) || (page >= (LCD_MAX_ROW - 16) && pos > (LCD_MAX_COLUMN - Font_FontWidth(&p_content[i])))
          || page > (LCD_MAX_ROW - 16) )
        {
            break;
        }
        ...
        if(pos + Font_FontWidth(&p_content[i]) > LCD_MAX_COLUMN)
        {
            rows_cnt++;
            page += 16 + 7;
            pos = LCD_FONT_INTERVAL;
        ...
        if(Font_TranslateLattice(&p_content[i], page, pos))
        {
            ...
            if(pos == LCD_MAX_COLUMN)
            {
                page += 16 + 7;
                pos = LCD_FONT_INTERVAL;
                rows_cnt++;
            }
            else if(pos > LCD_MAX_COLUMN)
            {
                page += 16 + 7;
                pos = Font_FontWidth(&p_content[i]);
                rows_cnt++;
                                 ...
                                 uint8_t *p_content, uint16_t content_len, uint16_t *start_page_index)
{
    ...
    for(i = 0; i < content_len_max; i += Font_CodeByteNum(&p_content[i]))
    {
        ...
        if(Font_IsDispSuc(&p_content[i]))
        {
            ...
            if((start_pos + Font_FontWidth(&p_content[i])) > LCD_MAX_COLUMN)
            {
                total_rows++;
