...
/**@file
 *
 * @defgroup ble_sdk_srv_clk Nordic UART Service - Experimental
 * @{
 * @ingroup  ble_sdk_srv
 ...
 *           by calling the ble_clk_on_ble_evt() function from the @ref ble_stack_handler callback.
 */
...
#define BLE_UUID_CLK_SERVICE            0x0001                       /**< The UUID of the Nordic UART Service. */
#define BLE_UUID_CLK_TX_CHARACTERISTIC  0x0003                       /**< The UUID of the TX Characteristic. */
#define BLE_UUID_CLK_RX_CHARACTERISTIC  0x0002                       /**< The UUID of the RX Characteristic. */

//#define BLE_CLK_MAX_DATA_LEN            (GATT_MTU_SIZE_DEFAULT - 3)  /**< Maximum length of data (in bytes) that can be transmitted by the Nordic UART service module to the peer. */

#define BLE_CLK_MAX_DATA_LEN            20  /**< Maximum length of data (in bytes) that can be transmitted by the Nordic UART service module to the peer. */

#define BLE_CLK_MAX_RX_CHAR_LEN         BLE_CLK_MAX_DATA_LEN         /**< Maximum length of the RX Characteristic (in bytes). */
#define BLE_CLK_MAX_TX_CHAR_LEN         20                          /**< Maximum length of the TX Characteristic (in bytes). */

// Forward declaration of the ble_clk_t type. 
typedef struct ble_clk_s ble_clk_t;

/**@brief Nordic UART Service event handler type. */
typedef void (*ble_clk_data_handler_t) (ble_clk_t * p_clk, uint8_t * data, uint16_t length);

/**@brief   Nordic UART Service init structure.
 *
 * @details This structure contains the initialization information for the service. The application
 *          needs to fill this structure and pass it to the service using the @ref ble_clk_init
 *          function.
 */
typedef struct
{
    ble_clk_data_handler_t   data_handler;            /**< Event handler to be called for handling received data. */
} ble_clk_init_t;
 ...
 *
 * @param[out]  p_clk       Nordic UART Service structure. This structure will have to be supplied
 ...
 *              This function returns NRF_ERROR_NULL if either of the pointers p_clk or p_clk_init
 *              is NULL.
 */
 ...
 *              peer.
  *
 * @param[in]   p_clk          Pointer to the Nordic UART Service structure.
 ...
 *              It returns NRF_ERROR_NULL if the pointer p_clk is NULL.
 */
uint32_t ble_clk_send_string(ble_clk_t * p_clk, uint8_t * string, uint16_t length);


uint32_t ble_clk_sending(ble_clk_t * p_clk, uint8_t * string, uint16_t length);
