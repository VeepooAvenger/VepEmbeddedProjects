...
static void on_disconnected(ble_evt_t const * p_ble_evt)
{
    ...
    {
        //WJ.2016.12.02 //YX.20171225.解决充电时清除数据，自动开机后，出现搜不到蓝牙的问题
        if(p_data_manage->device_trace_param_s.sign_on == 0x01)   //Broadcast shutdown under no longer open
        {
...
//WJ.20170214,解决一段时间后不广播的问题
void restart_adv_slow(void)
{
    sd_ble_gap_adv_stop();
