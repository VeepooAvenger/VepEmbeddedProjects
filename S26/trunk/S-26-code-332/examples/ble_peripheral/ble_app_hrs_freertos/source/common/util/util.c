...
void distance_value(uint32_t *step_input,uint32_t *meter,float *kilometer,float *mile, float *calories)
{
    float temp = 0; 
...
// 获取血氧有效平均值
uint8_t Ox_FilterAverage(uint8_t *ox_array, uint16_t ox_len)
{
    uint16_t ox_vailed_len = 0;
    uint32_t ox_sum = 0;
    if(ox_array == NULL || ox_len == 0)
    {
        return 0;
    }
    for(uint16_t i = 0; i < ox_len; i++)
    {
        if(ox_array[i] > 10 && ox_array[i] <= 100)
        {
            ox_sum += ox_array[i];
            ox_vailed_len++;
        }
