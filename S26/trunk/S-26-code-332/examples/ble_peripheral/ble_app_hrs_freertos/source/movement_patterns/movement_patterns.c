...
//extern void Ble_BatTimeOutAutoRepeat(uint8_t* p_data,uint8_t len);
/****************************************************************
...
{ 
	...
	//YX.20180806.修复运动模式初始化，未调用4404校准算法
...
{
    //ZJH.20180611.添加心率测量标志位设置，避免APP测量应答错误的测量状态
...
//    flash_block_usable = App_FlashCheckAreaIsUsable((RECORD_DAILY_BASE_ADDR+RECORD_MOVEMENT_OFFSET)/1024 + movement_info_s.block * 2, 2);   
...
{    
    ...
    movement_info_s.ui_heart = p_data_manage->heart_measure_s.real_heart;                  //实时心率
...
{        
//    BleThread_SaveMovementHead();
    App_FlashWriteMovementHead(movement_info_s.block, (uint8_t *)&movement_head_s, sizeof(movement_patterns_head_t));
