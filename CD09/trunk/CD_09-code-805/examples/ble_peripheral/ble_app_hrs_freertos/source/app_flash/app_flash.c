...
uint8_t App_FlashReadDailyData(uint8_t block_count, uint16_t number, uint8_t *data)
{
    ...
    if(block_count >= DAILY_DATA_BLOCK_MAX || number >= DAILY_PKG_MAX_NUM)
    {
...
uint8_t App_FlashWriteSystemDataRecord(system_record_info_t system_record_s, const uint8_t *data, const uint16_t data_len)
{
    ...
    static uint32_t  m_deadbeef[3][SYSTEM_DATA_EACH_WORDS_SIZE] = {0,};        // 120 byte
...
/****************************************************************
** Function name:           App_FlashEraseAllData
** Descriptions:            
** input parameters:
** output parameters:
** Returned value:
...
uint8_t App_FlashEraseAllData(void)
{
    fs_ret_t  error_code = FS_SUCCESS;
    //女性和闹钟和系统参数
    uint32_t no_del_page = ALARM_DATA_BLOCK_UINT + PHYS_PERIOD_DATA_TOTAL_PAGE + 1;
    
    error_code = App_FlashErasePage(&app_fs_config, app_fs_config.p_start_addr + PHYS_PERIOD_DATA_TOTAL_PAGE*1024,
                                   ALL_FLASH_PAGE_SIZE - no_del_page, NULL);  
...
/****************************************************************
** Function name:           App_FlashEraseCheck
** Descriptions:            檫除检查
** input parameters:
** output parameters:
** Returned value:
