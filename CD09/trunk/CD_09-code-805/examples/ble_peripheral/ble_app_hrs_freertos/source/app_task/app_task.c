...
low_energe_protocol_data_t low_energe_protocol_data = {0};
power_statistics_t power_statistics_s = {0};
...
void App_SystemFlagInit(void)
{
    ...
    Product_GetLowEnergeFunction(&low_energe_protocol_data);
    
    //ZJH.20190725.开机上电对低功耗控制参数进行检查
    if(App_CheckLowEnergeFunctionHandler() != true)
    {
        App_LowEnergeFunctionInitHandler();
    }
    
    
    if(low_energe_protocol_data.state != 0x02)
    {
        low_energe_protocol_data.state = 0x01;
    }
    
    if(FUNCTION_SUPPORT_LOW_ENERGE == 0)
    {
        low_energe_protocol_data.state = 0x00;
    }
        ...
        if(system_flag_s.sign_off != 0x01 && system_flag_s.ota_encrypt)
        {
            switch(t_event)
            {
                ...
                case APP_DEV_SET_SIT_FUNCTION_EVENT:
                    ...
                    break;
                case APP_DEV_SET_LOW_ENERGE_FUNCTION_EVENT:
                    App_DeleteEvent(t_event);
                    App_SetLowEnergeFunctionHandler();
                    break;
...
//        Acc_StartHandler();    
    ...
    {
        power_statistics_s.ble_connection_time++;
    }    
}
...
static void App_BatteryChargingMonitor(void)
{
    ...
    //WJ.20190325.解决充电状态下先亮屏再显示UI的BUG
