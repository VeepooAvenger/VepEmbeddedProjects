...
void Phone_NotifyCore(void)
{
    ...
    if((message_p->call_enable == 0x01) && (phone_notify_s.hint.call == 0x01))
    {
        ...
        if(low_energe_protocol_data.state == 1 && mt_cnt >= 1)
        {
             Motor_AddEvent(20, 1, 1*mt_cnt, 2*8);//提醒20组 非持续振动 振动1秒 间隔2秒     
           
        }
        else
        {
           Motor_AddEvent(20, 1, 1*8, 2*8);//提醒20组 非持续振动 振动1秒 间隔2秒     
        }
    ...
    if(low_energe_protocol_data.state == 1 && mt_cnt >= 1)
    {
        Motor_AddEvent(1, 0, 1*mt_cnt, 0);//提醒1组 非持续振动 振动1秒 
    }
    else
    {
        Motor_AddEvent(1, 0, 1*8, 0);//提醒1组 非持续振动 振动1秒 
    }
}
