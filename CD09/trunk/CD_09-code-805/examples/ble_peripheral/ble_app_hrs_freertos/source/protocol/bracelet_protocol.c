...
/****************************************************************
** Function name:           Protocol_LowEnerge
** Descriptions:            低功耗协议设置
...
**----------------------------------------------------------------
*****************************************************************/ 
static void Protocol_LowEnerge(ble_nus_t *p_nus, uint8_t *p_data)
{  
    ...
    if(p_data[2] == 0x01 ) //设置
    {
        //ZJH.20190725.如果APP发过来的参数超出有效值，则返回0x00失败
        if( (p_data[3] > 2 ) ||
        ((p_data[4] & 0x3F) < 1) || ((p_data[4] & 0x3F) > 64) ||
        ((p_data[5] & 0x3F) < 1) || ((p_data[5] & 0x3F) > 64) ||
        ((p_data[6] & 0x3F) < 1) || ((p_data[6] & 0x3F) > 64) ||
        ((p_data[7] & 0x3F) > 10) ||
        ((p_data[8] & 0x3F) < 1) || ((p_data[8] & 0x3F) > 10) )
        {
            ble_ack[3] = 0x00;
        }
        else 
        {
            if(p_data[3] == 0x01)
            {
                 low_energe_protocol_data.state = 1; //开启低功耗
            }
            else
            {
                 low_energe_protocol_data.state = 2; //关闭低功耗
            }
           
            low_energe_protocol_data.notice_time_value = (p_data[4] & 0x3F);
            
            low_energe_protocol_data.posture_time_value = (p_data[5] & 0x3F);
        
            low_energe_protocol_data.normal_light_time_value = (p_data[6] & 0x3F);

            low_energe_protocol_data.motor_delay_time_value = (p_data[7] & 0x3F);
           
            low_energe_protocol_data.moter_time_value = (p_data[8] & 0x3F);
            
            App_AddEvent(APP_DEV_SET_LOW_ENERGE_FUNCTION_EVENT);
        
            ble_ack[3] = 0x01;
            memcpy(&ble_ack[4], &low_energe_protocol_data, sizeof(low_energe_protocol_data));
        }
    }
...
static void Protocol_AlarmClockSet(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(0x10 == p_data[10])      //立即响应振动
    {
        ...
        Motor_AddEvent(6, 1, MOTOR_TIME_06, 1*8);//提醒六组 每组振动1秒 间隔1秒               
#endif
...
static void Protocol_SendDailyData(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_data[3] < 5)
    {
        ...
        if(daily_falg == 0)
        {
            ...
            if(all_pkg > DAILY_PKG_MAX_NUM - 2)
            {
                all_pkg = DAILY_PKG_MAX_NUM - 2;
            }
            
            if(start_num != all_pkg && all_pkg <= DAILY_PKG_MAX_NUM-2 && all_pkg >= 1)
            {
                start_num -= 1;//WJ.20170420.修复日常数据传输不正确，只传一部分
...
{
    uint8_t ble_ack[22] = {0x00,};
    
    uint8_t  read_data[20] = {0x00,};
    ...
    if(p_data[1] == 0x01)
    {
        ...
        {        
            ...
            if(day_id == 0x00)
            {  //当天
                memcpy(&p_power_temp, &power_statistics_s, sizeof(p_power_temp));
                // 亮屏次数
                ble_ack[4] = p_power_temp.bright_screen_num & 0xFF;            
                ble_ack[5] = (p_power_temp.bright_screen_num>>8) & 0xFF; 
                // 亮屏时长
                ble_ack[6] = p_power_temp.bright_screen_time & 0xFF;            
                ble_ack[7] = (p_power_temp.bright_screen_time>>8) & 0xFF; 
                // 马达次数
                ble_ack[8] = p_power_temp.motor_vibration_num & 0xFF;            
                ble_ack[9] = (p_power_temp.motor_vibration_num>>8) & 0xFF; 
                // 马达时长
                p_power_temp.motor_vibration_time /= 8;
                ble_ack[10] = p_power_temp.motor_vibration_time & 0xFF;            
                ble_ack[11] = (p_power_temp.motor_vibration_time>>8) & 0xFF;   
                // 蓝牙连接时长
            ...
            {
                block = Daily_GetPreviousBlock(day_id);
                App_FlashReadDailyData(block, DAILY_PKG_MAX_NUM - 2, (void *)read_data); 
                
                // 限制长度，防止后期扩展越界
                memcpy(&p_power_temp, read_data, 16);
                if(p_power_temp.bright_screen_num == 0xFFFF 
                  || p_power_temp.bright_screen_time == 0xFFFF)
                {   //数据不存在
                    ble_ack[2] = 3;
                }
                else
            ...
            else if(day_id == 5 && p_data[2] == 0xFE)
            {
                ble_ack[3] = 0xFE;
            }
            else
...
void Protocol_BatDataHandler(ble_nus_t *p_nus, uint8_t *p_data1, uint16_t length)
{
    ...
    switch(p_data[0])
    {
		case 0x03:
			//设置低功耗协议
            Protocol_LowEnerge(p_nus, p_data);
            break;
        case 0x80:
