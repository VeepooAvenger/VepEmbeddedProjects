...
static ble_ancs_c_parse_state_t attr_data_parse(ble_ancs_c_t * p_ancs, const uint8_t * p_data_src, uint32_t * index)
{
    ...
    if (   (p_ancs->current_attr_index < p_ancs->ancs_attr_list[p_ancs->evt.attr.attr_id].attr_len - 1) //WJ.20170417
    ...
    if ( (p_ancs->current_attr_index == p_ancs->evt.attr.attr_len) ||
         (p_ancs->current_attr_index == p_ancs->ancs_attr_list[p_ancs->evt.attr.attr_id].attr_len - 1))//WJ.20170417
    {
                        ...
                        const uint16_t       hvx_data_len)
{
    ...
    if(ancs_evt.notif.category_id == BLE_ANCS_CATEGORY_ID_INCOMING_CALL)
    {
        ...
        {
            Ancs_SetBlePhone(0x01);  
        }
        else
        { 						
            Ancs_SetBlePhone(0x00);
            Motor_DelEvent(); //马达停止振动
            
            if(Phone_NotifyGetCallingState() == PHONE_CALL_READYREJECT)//设备主动挂断
            {
    ...
    if((ancs_evt.notif.category_id == BLE_ANCS_CATEGORY_ID_SOCIAL) && (ancs_evt.notif.evt_id == BLE_ANCS_EVENT_ID_NOTIFICATION_ADDED))
    {
...
/****************************************************************
** Function name:           encode_notif_action
** Descriptions:            对数据编码
** input parameters:
** output parameters:
** Returned value:
...
**----------------------------------------------------------------
*****************************************************************/ 
static uint16_t encode_notif_action(uint8_t * p_encoded_data, uint32_t uid, uint8_t  action_id)
{
    uint8_t index = 0;
...
/****************************************************************
** Function name:           ble_ancs_c_perfor_attrs
** Descriptions:            对来电接听、挂断操作入口
** input parameters:
** output parameters:
** Returned value:
** Created by:              WJ             
...
**----------------------------------------------------------------
*****************************************************************/ 
uint32_t nrf_ancs_perform_notif_action(ble_ancs_c_t * p_ancs, uint32_t uid, uint8_t action_id)
{
    VERIFY_PARAM_NOT_NULL(p_ancs);
 
    uint16_t len = 0;
    ancs_tx_message_t * p_msg = NULL;

    p_ancs->number_of_requested_attr = 0;
    p_msg              = &m_tx_buffer[m_tx_insert_index++];
    m_tx_insert_index &= TX_BUFFER_MASK;        //WJ.20170802.解决可能出现越界问题
    
    memset(p_msg, 0, sizeof(ancs_tx_message_t));


    len = encode_notif_action(p_msg->req.write_req.gattc_value, uid, action_id);
