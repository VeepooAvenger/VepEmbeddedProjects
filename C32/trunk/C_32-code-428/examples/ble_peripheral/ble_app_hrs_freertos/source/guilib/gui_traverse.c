...
void Cpu_TaskMainTest(cpu_task_controller_t *p_controller, gui_controller_t *p_gui)
{
	...
	{
        /* UI遍历中 */
        TASK_ENTRY_INIT(p_controller, 12);
        p_controller->key_press_hold = 0;
        traverse_tmp_buf1 = 0;
        traverse_tmp_buf2 = 0;
        traverse_tmp_buf3 = 0;
...
//            if(task_interval_cnt % 5 == 1)
//            {
//                task_interval_cnt++;
//            }
            
                  
            if((traverse_tmp_buf1 > 99)&&(traverse_tmp_buf3 > 99))
            {
                traverse_tmp_buf1 = 0;
                traverse_tmp_buf3 = 0;
                traverse_tmp_buf4++;
                if((traverse_tmp_buf4%5) == 2) 
                {
                    traverse_tmp_buf4++;
                }
...
//                   main_background_color=4;   
            if((traverse_tmp_buf4%5) == 0)  
            {                
...
//            }
            ...
            Gui_PageMainAssisant(task_interval_cnt, ble_con_state, time_format, battery_charging, battery_level, display_language, 0);
            Gui_PageMainDateHour(task_interval_cnt, month, day, hour, minute, week, display_language, time_format, weather, 
