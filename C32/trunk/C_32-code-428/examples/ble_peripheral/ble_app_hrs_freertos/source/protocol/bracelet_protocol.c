...
static void Protocol_OledSet(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_data[1] == 0x01)
    {
        ...
        { 
            ...
            else
            {
                if(p_data[8] == 1)
                {   //自动
                    system_info_s.brightness_leve.leve = CONFIG_BACKLIGHT_MAX_LEVEL_RECOMMEND;
                    system_info_s.brightness_leve.setleve = CONFIG_BACKLIGHT_MIN_LEVEL_RECOMMEND;
    ...
    if(system_info_s.brightness_leve.s_hour == 22 
       && system_info_s.brightness_leve.e_hour == 8)
    {
        ble_ack[9] = 1;    
    } 
...
static void Protocol_BrightScreenTimeSet(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_data[1] == 0x01)
    {
        ...
        {
            if(system_info_s.ui_display_time != p_data[2])
            {
                system_info_s.ui_display_time = p_data[2];     
                App_SetSystemInfoSave();   
            }    
        }        
    }
    
    ble_ack[3] = system_info_s.ui_display_time;
...
static void Protocol_Android_NotifySet(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(p_data[1] == 0x01)       //手机打开0x01
    {
        ...
        { 
//             Motor_AddEvent(1, 0, 0.5 * 20, 0);//提醒一组 每组振动1秒 间隔2秒
        } 
    }
...
static void Protocol_TimeChange(uint8_t *time_data)
{
    ...
    //WJ.20190618.跨一年对时不清楚数据的问题；
    t_year = time_data[1] + (time_data[0] << 8);
    c_year = clock->year;
    
    Sclock_SynchronizationTime(time_data, 7);
 
    if(month != time_data[2] || day != time_data[3] || c_year != t_year)
    {
...
static void Protocol_GestureSwitch(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    ble_ack[9] = 4; //C-32_428客户要求等级默认为4
    Ble_SendDataViaBat(ble_ack, 20);    
}
...
static void Protocol_AlarmClockSet(ble_nus_t *p_nus, uint8_t *p_data)
{
    ...
    if(0x10 == p_data[10])      //立即响应振动
    {
        ...
        Motor_AddEvent(6, 1, 1 * 20, 1 * 20);//提醒六组 每组振动1秒 间隔1秒               
#endif
      ...
      && p_data[4] == 0x73 && p_data[5] == 0x40)
    {
        ...
        if(ble_ack[1] == 0x01 || ble_ack[1] == 0x02)
        {
            if(system_flag_s.bp_flag != ble_ack[1])
            {
                ...
                App_AddEvent(APP_DEV_SETBP_FUNCTION_EVENT);
            }
        }
