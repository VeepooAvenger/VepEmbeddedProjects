...
#define ACC_SAMPLE_INTERVAL             200//200ms                            /**< acceleration measurement interval (ms). */
#define ACC_SAMPLE_TIME_CNT             300//40//20
...
uint16_t acc_data_buf_cnt = 0;                  //缓存数据个数
...
void Acc_Calculate(void)
{
    ...
    acc_data_buf_cnt = Gsensor_GetBufferCurrentByte();
    
    acc_data_buf_cnt= acc_data_buf_cnt/6; 
    if(acc_data_buf_cnt == 0) 
    {
        i = 0;
        acc_error_cnt++;
        return ;
    ...
    if(acc_data_buf_cnt >  ACC_SAMPLE_BUF_MAX_CNT)
    {
        acc_data_buf_cnt = ACC_SAMPLE_BUF_MAX_CNT;
    }       
    
    ret = Gsensor_ReadoneSample((int16_t *)&acc_data_buf[0][0], acc_data_buf_cnt*6);   
    if(!ret)
    {
        i = 0;
        acc_error_cnt++;
    ...
    acc_error_cnt = 0;
    for(i = 0; i < acc_data_buf_cnt; i++)
    {    
        memcpy(acc_data, &acc_data_buf[i][0], 6);
        ...
        {
            gesture_cnt = 0;
            Gesture_Detection16bit(pyData, pxData, pzData, pvData, system_info_s.gesture_level);//5);//3);
            Acc_GestureDeal(); //根据姿态进行处理
        } 
        data[3] = 0;
        Pedometer_Process(&data[0], acc_count++, 8);
        if(acc_count >= 25)
        {
            acc_count = 0;
        }
...
static void Acc_TimeroutHandler(TimerHandle_t xTimer)
{
    ...
    acc_lock_state = 1;
    if(Bps_Spi0SelectIsBusy())    //SPI忙碌
    {
        acc_lock_state = 0;
        return;
    }

    if(Gsensor_Init1State())// && acc_cnt < 10)  
	{    
        acc_cnt = ACC_SAMPLE_TIME_CNT;		//8s
...
{ 
    ...
    if(!Gsensor_Check() || acc_error_cnt >= 5)    //自检配置不通过或者连续5次没有读到数据
    {
        Acc_TimerStop();      
        Gsensor_AccInit(); 
        acc_startflag = 0;
...
{   
    ...
    static uint8_t acc_check_time_cnt = 0;    
    static uint8_t acc_lock_cnt = 0;          
    
    if(acc_lock_state == 1) //加速度数据处理中断已经开始，资源互斥处理
    {
        if(acc_lock_cnt++ > 30)
        {
            acc_lock_state = 0;
    ...
    acc_lock_cnt = 0;
    if(acc_startflag == 0 && Gsensor_Init1State())  
	{
        if(Bps_Spi0SelectIsBusy())    //SPI忙碌
        {
            return;
	...
	else
	{
        ...
        {
            if((++last_acc_same_cnt) >= 2)     //定时器异常停止工作，清除状态
            {
                Acc_TimerStop(); 
                //acc初始化
                Gsensor_Configure();  
        ...
        else if(acc_time_flag == 1)
        {            
            last_acc_same_cnt = 0;
        }
        else
        {          
            if(acc_check_time_cnt++ > 30 || acc_error_cnt >= 5)   //加速度不工作后30s做一次自检或者连续5次没有读到数据
            {    
                if(Bps_Spi0SelectIsBusy())    //SPI忙碌
                {
                    return;
...
{
    if(acc_cnt >= ACC_SAMPLE_TIME_CNT - 12)//30)
    {
        return 1;
    }        
    return 0;
