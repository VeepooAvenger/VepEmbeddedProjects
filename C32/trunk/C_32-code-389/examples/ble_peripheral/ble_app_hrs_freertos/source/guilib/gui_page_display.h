...
void Gui_PageMainDateHour1(uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t week,
                          ...
                          uint8_t temperature_high);
//显示主界面其他图标
void Gui_PageMainAssisant(uint8_t index, uint8_t ble_status, uint8_t am_pm_format, uint8_t battery_status, 
                          uint8_t battery_level, uint8_t language, uint8_t menstrual);
...
//显示消息推送界面
void Gui_PageMessageNotificationContent(uint8_t *display_buf, uint16_t buf_len, uint8_t index, uint8_t msg_type);
