...
typedef struct
{
	// posi_steps = steps_total = All_StepGet
	uint32_t posi_steps;    
	uint32_t posi_cal;
...
** input parameters:       pp_data_input[3]: include x y z / pp_len:input number count get_sensor_set
** output parameters:      none
** Returned value:         none
** Created by:             HYR
** Created Date:           20161013
*******************************************************************************/
void Pedometer_Process(int16_t *pp_data_input, int16_t pp_len, uint8_t get_sensor_set);
