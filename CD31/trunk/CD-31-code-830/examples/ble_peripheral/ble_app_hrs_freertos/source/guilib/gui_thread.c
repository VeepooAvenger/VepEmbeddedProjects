...
uint8_t ui_style_type = 0;                                  //界面风格
uint8_t gui_display_time = 0;                               //UI界面显示时长，部分界面不可设置

uint8_t delay_time_cnt = 0;
uint8_t ui_change_flag = 0;                      //运动模式下灭屏操作

uint8_t task_return_flag = 0;					// 任务被事件打断标志
...
static uint8_t posture_to_bright_flag = 0;      //姿态亮屏的标志

uint8_t lcd_backlight_level = 0;				//lcd背光等级
...
//            {
//                //WJ.20180704.增加关机后TP休眠，不能TP唤醒
//                Gui_TpSleep(SLEEP_MODE_6);     //TP休眠
//                //Tp_ShutDown();
//            }
//            else
//            {
//                //WJ.20180704.增加TP休眠
//                Gui_TpSleep(SLEEP_MODE_1);     //TP休眠                
//            }
        }
	}
...
//           || cpu_s.task == CPU_TASK_MOVEMENT_PAUSE || cpu_s.task == CPU_TASK_MOVEMENT_EXIT)
        //YX.20181017.解决运动模式亮屏时，有些功能没有屏蔽的问题
