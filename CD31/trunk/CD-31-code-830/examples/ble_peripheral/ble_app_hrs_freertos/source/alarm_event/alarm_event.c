...
{ 
    ...
    {   //app开启单次倒计时,
        //YX.20181017.解决进入倒计时后，还会显示其他任务界面的问题，然后按键会切换到其他界面
        //界面为运动模式显示界面，查找手机，秒表显示时，不进行清除界面
        if(!(Gui_GetThreadTask() == CPU_TASK_MOVEMENT_PAUSE
           || Gui_GetThreadTask() == CPU_TASK_MOVEMENT_EXIT
           || Gui_GetThreadTask() == CPU_TASK_MOVEMENT_DATA
           || (Gui_GetThreadTask() == CPU_TASK_FINDPHONE && Gui_GetThreadAnimationEnable())
           || (Gui_GetThreadTask() == CPU_TASK_STOP_WATCH && Gui_DataStopWatchIsBusy()))
           )
        {
            Gui_ThreadTaskClear();
        }
