...
daily_heart_average_t daily_heart_average_s = {0};  //WJ.20180808.半个小时心率的均值
...
{
    if(cur_pkg_info.pkg_cnt < DAILY_PKG_MAX_NUM)
    {
...
void Daily_NextDay(void)
{
    ...
    Daily_ClearHeartAverageEveryHalfHour();
}
        ...
        {           
            for(j = 0; j < DAILY_PKG_MAX_NUM; )
            {
                ...
                if(daily_data_temp.day != 0xFF && daily_data_temp.month != 0xFF)
                {
                    ...
                    if(daily_data_temp.heart >= 30 && daily_data_temp.heart <= 200)
                    {
                        system_flag_s.display_heart = daily_data_temp.heart;
                    }
...
/****************************************************************
** Function name:           Daily_CalcHeartAverageEveryHalfHour
** Descriptions:            统计半个小时内心率的平均值
** input parameters:        无
** output parameters:       无
** Returned value:          无
** Created by:              WJ              
...
*****************************************************************/ 
void Daily_CalcHeartAverageEveryHalfHour(void)
{   
    uint8_t     index = 0;
    uint8_t     c_back_id = 0;
    uint16_t    c_pack_cnt = 0;
    uint16_t    heart_sum = 0;
    daily_data_format_t  daily_data_temp = {0};  
    
    daily_heart_average_t temp_heart_average_s = {0};
    
    c_back_id = cur_pkg_info.block;
    c_pack_cnt = cur_pkg_info.pkg_cnt;
    
    for(uint16_t i = 0; i < c_pack_cnt; i++)
    {
        App_FlashReadDailyData(c_back_id, i, (void *)&daily_data_temp); 
        if(daily_data_temp.heart >= 30 && daily_data_temp.heart <= 200)
        {
            if(daily_data_temp.hour < 24)
            {
                index = daily_data_temp.hour * 2;
                if(daily_data_temp.minute >= 30)
                {
                   index++; 
                }
                heart_sum = temp_heart_average_s.heart_arg[index] * temp_heart_average_s.heart_cnt[index];
                heart_sum += daily_data_temp.heart;
                temp_heart_average_s.heart_cnt[index]++;
                temp_heart_average_s.heart_arg[index] = heart_sum / temp_heart_average_s.heart_cnt[index];
            }
        }
    }  
    ...
    {
        if(temp_heart_average_s.heart_arg[i] > temp_heart_average_s.max_heart)   
        {
            temp_heart_average_s.max_heart = temp_heart_average_s.heart_arg[i];
        }
        
        //WJ.20180930.获取心率的最小值错误问题
        if(temp_heart_average_s.heart_arg[i] != 0 && temp_heart_average_s.min_heart == 0)
        {
            temp_heart_average_s.min_heart = temp_heart_average_s.heart_arg[i];
        }       
        if(temp_heart_average_s.heart_arg[i] != 0
          && temp_heart_average_s.heart_arg[i] < temp_heart_average_s.min_heart)
        {
            temp_heart_average_s.min_heart = temp_heart_average_s.heart_arg[i];
        }
    }
    ...
    memcpy(&daily_heart_average_s, &temp_heart_average_s, sizeof(daily_heart_average_s));
}

/****************************************************************
** Function name:           Daily_ClearHeartAverageEveryHalfHour
** Descriptions:            清除半个小时内心率的平均值
** input parameters:        无
** output parameters:       无
** Returned value:          无
** Created by:              WJ              
** Created Date:            2018-08-08
*****************************************************************/ 
void Daily_ClearHeartAverageEveryHalfHour(void)
{   
    memset(&daily_heart_average_s, 0, sizeof(daily_heart_average_s));
}
