...
void Sleep_SaveData(void)
{
        ...
        || p_sleep_info->enter_time.date != clock->day)
    {
        ...
        {
            //
            if(daily_block_info.block_flag == 0xFF)
            {
                daily_block_info.pack_num = 0;
                daily_block_info.all_calories = 0;
                daily_block_info.all_distance = 0;
                daily_block_info.all_step[0] = 0;
                daily_block_info.all_step[1] = 0;
                daily_block_info.all_step[2] = 0;
            }
...
/****************************************************************
** Function name:           Sleep_GetTimeInfo
...
*****************************************************************/ 
uint16_t Sleep_GetTimeInfo(uint16_t *all_time, uint16_t *shallow_sleep_time, 
                           uint16_t *deep_sleep_time, uint8_t *quality)
{
    ...
    if(daily_block_info.block_flag == 0x00)
    {
        ...
        for( i = 0; i < curr_sleep_cnt; i++)
        {
            ...
            {
                sleep_total_minute += sleep_quality_info.total_time;
                //WJ.20180808.增加获取睡眠质量和深浅睡眠时间
                deep_time += sleep_quality_info.sleep_quality[0];
                shallow_time += sleep_quality_info.sleep_quality[1];
                
                if(sleep_quality_info.sleep_quality[2] > sleep_quality)
                {
                    sleep_quality = sleep_quality_info.sleep_quality[2];
                }
            }
            else
            {
    ...
    if(all_time)
    {
        *all_time = sleep_total_minute * 5;
    }
    if(shallow_sleep_time)
    {
        *shallow_sleep_time = shallow_time * 5;
    }
    if(deep_sleep_time)
    {
        *deep_sleep_time = deep_time * 5;
    }
    if(quality)
    {
