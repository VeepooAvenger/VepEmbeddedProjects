...
typedef enum
{
	...
	BUTTON1_EVENT_CLICK,
	BUTTON1_EVENT_LONGPRESS,
    BUTTON1_EVENT_LONGPRESS_6S,
}BUTTON_EVENT_TYPE;
...
typedef enum
{
	...
	CPU_TASK_TURNOFF,		  //任务--带图标确认的关机
	CPU_TASK_POWEROFF,        //任务--关机
	CPU_TASK_POWERON,         //任务--开机
       
    CPU_TASK_MAC_VER,         //任务--显示MAC和VER
    
	CPU_TASK_MOVEMENT_PAUSE,  //任务--运动模式暂停   
	CPU_TASK_MOVEMENT_EXIT,   //任务--运动模式退出  
	CPU_TASK_MOVEMENT_DATA,    //任务--运动模式数据界面
...
typedef enum
{
	...
	CPU_EVENT_TASK_CALL_REMIND,       // 事件任务--来电
...
typedef struct
{
    uint8_t         key;            //触摸按键的状态：0：空 ...
    uint16_t        slid_dist;      //滑动效果时使用，滑动的距离
    point_coord_t    point_info_s;   //单击时使用，点的坐标
}tp_key_info;
